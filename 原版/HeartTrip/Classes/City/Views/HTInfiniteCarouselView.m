...
{
    NSLog(@"重写HTInfiniteCarouselView.ImageURLSignal的setter方法，");
    _imageURLSignal = imageURLSignal;
    @weakify(self);
    NSLog(@"HTInfiniteCarouselView.ImageURLSignal 订阅信号");
    [_imageURLSignal subscribeNext:^(id  _Nullable x) {
       NSLog(@"HTInfiniteCarouselView.ImageURLSignal的订阅信号block中，查看数据，bannerData数据发生改变，监听信号改变执行订阅者block");
        @strongify(self);
        if ([x count] > 0) {
            
            [self.placeholderImageView removeFromSuperview];
            NSLog(@"HTInfiniteCarouselView.ImageURLSignal的订阅信号block中，解包数据");
            [x enumerateObjectsUsingBlock:^(HTBannerModel *model, NSUInteger idx, BOOL * _Nonnull stop) {
                 NSLog(@"HTInfiniteCarouselView.ImageURLSignal的订阅信号block中，数据赋值");
                [self.imageURLStringsGroup addObject:model];
            }];
        }
        
        dispatch_main_async_safe(^{
            NSLog(@"HTInfiniteCarouselView.ImageURLSignal的订阅信号block中，GCD回到主队列，三方轮播控件刷新数据");
