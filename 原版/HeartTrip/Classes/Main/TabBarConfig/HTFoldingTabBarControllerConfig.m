...
- (YALFoldingTabBarController *)flodingTabBarController
{
    return HT_LAZY(_flodingTabBarController, ({
        ...
        tabBarVC.viewControllers = [self viewControllersForController];//tab添加子控制器
...
- (NSArray *)viewControllersForController {
    ...
    // 数据服务
    self.viewModelService = [[HTViewModelServicesImpl alloc] initModelServiceImpl];//创建遵守协议的各子界面网络层
    
    // 首页
    self.cityTravelViewModel = [[HTCityTravelViewModel alloc] initWithServices:self.viewModelService params:nil]; //init方法让HTViewModel开始创造命令，开始监听
