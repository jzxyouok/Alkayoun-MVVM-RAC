...
#import "RWCustomTabBarController.h"
...
#pragma mark - NSObject+RWTabBarControllerItem

@interface NSObject (RWTabBarControllerItemInternal)

- (void)RW_setTabBarController:(RWCustomTabBarController *)tabBarController;

@end

@implementation NSObject (RWTabBarControllerItemInternal)

-(void)RW_setTabBarController:(RWCustomTabBarController *)tabBarController
{
    objc_setAssociatedObject(self, @selector(RW_tabBarController), tabBarController, OBJC_ASSOCIATION_ASSIGN);
}

@end

////*******************************************************************************************
@implementation NSObject (RWCustomTabBarController)

-(RWCustomTabBarController *)RW_tabBarController
{
    RWCustomTabBarController *tabBarController = objc_getAssociatedObject(self, @selector(RW_tabBarController));
    
    if (!tabBarController) {
        if ([self isKindOfClass:[UIViewController class]] && [(UIViewController *)self parentViewController]) {
            tabBarController = [[(UIViewController *)self parentViewController] RW_tabBarController];
        }else{
            id<UIApplicationDelegate> delegate = ((id<UIApplicationDelegate>)[[UIApplication sharedApplication] delegate]);
            UIWindow *window = delegate.window;
            if ([window.rootViewController isKindOfClass:[RWCustomTabBarController class]]) {
                tabBarController = (RWCustomTabBarController *)window.rootViewController;
            }
        }
    }
...
////*******************************************************************************************
@interface RWCustomTabBarController ()<UITabBarControllerDelegate>

@end

@implementation RWCustomTabBarController
...
- (void)setViewControllers:(NSArray<__kindof UIViewController *> *)viewControllers
{
    ...
    if (viewControllers && [viewControllers isKindOfClass:[NSArray class]]) {
        if ((!_tabBarItemsAttributes)||(_tabBarItemsAttributes.count != viewControllers.count)) {
            [NSException raise:@"RWCustomTabBarController" format:@"设置_tabBarItemsAttributes属性时，确保元素个数与控制器的个数相同，并在方法`-setViewControllers:`之前设置"];
        }
        ...
        for (UIViewController *viewController in _viewControllers) {
            ...
            [self addOneChildViewController:viewController WithTitle:title normalImageName:normalImageName selectedImageName:selectedImageName];
            [viewController RW_setTabBarController:self];
            idx++;
        }
    }else{
        for (UIViewController *viewController in _viewControllers) {
            [viewController RW_setTabBarController:nil];
        }
...
+ (instancetype)tabBarControllerWithViewControllers:(NSArray<UIViewController *> *)viewControllers tabBarItemsAttributes:(NSArray<NSDictionary *> *)tabBarItemsAttributes {
    RWCustomTabBarController *tabBarController = [[RWCustomTabBarController alloc] initWithViewControllers:viewControllers tabBarItemsAttributes:tabBarItemsAttributes];
...
+ (NSUInteger)allItemsTabBarCount{
    ...
    if ([RWCustomTabBarController haveCustomButton]) {
        allItemsInTabBar += 1;
    }
...
- (void)offsetTabBarSwappableImageViewToFit:(CGFloat)swappableImageViewDefaultOffset {
    ...
    NSArray<UITabBarItem *> *tabBarItems = [self RW_tabBarController].tabBar.items;
